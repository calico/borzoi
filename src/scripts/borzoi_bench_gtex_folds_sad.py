...
Benchmark Borzoi model replicates on GTEx eQTL classification task (gene-agnostic).
"""
...
def main():
    ...
    for ci in range(options.crosses):
        for fi in fold_index:
            ...
            cmd_fold = '%s time borzoi_sad.py %s %s %s' % (
    ...
    cmd_base = 'borzoi_bench_classify.py -i 100 -p 2 -r 44 -s'
    ...
    cmd_base = 'borzoi_gtex_coef_sad.py -g %s --susie %s' % (options.gtex_vcf_dir, options.susie_dir)
