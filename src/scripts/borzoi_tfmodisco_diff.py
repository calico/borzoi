...
Run TF Modisco on difference between borzoi input saliency scores for multiple tissues.
'''
...
def main():
    ...
    parser.add_option(
        ...
        default=None,
        type='int',
        help='Main tissue index in list of tissue files.',
    )
    parser.add_option(
        '--gene_file',
        dest='gene_file',
    ...
    if len(args) != 0:
        parser.error('You must not supply any arguments.')
    ...
    for scores_h5_file in options.tissue_files :
    ...
    if options.main_tissue_ix is None :
        tissue_ix = -1
        for h5_ix, scores_h5_file in enumerate(options.tissue_files) :
            if options.tissue.split("_")[0] in scores_h5_file :
                tissue_ix = h5_ix
                break
    else :
        tissue_ix = options.main_tissue_ix
    
    print('tissue_ix = ' + str(tissue_ix))
    
    score_2 = hyp_scores[tissue_ix, ...]
    score_1 = np.mean(hyp_scores[np.arange(len(options.tissue_files)) != tissue_ix, ...], axis=0)

    hyp_scores = score_2 - score_1
    ...
    if options.norm_type == 'max':
        ...
        max_clip = np.percentile(scores_max, 25)
        ...
        wide_clip = np.percentile(scores_std_wide, 25)
