...
def main():
    ...
    bench_options.add_option(
        "-b", dest="bench_dir", default="/home/drk/seqnn/data/crispr/flowfish"
    )
    bench_options.add_option(
        "-e",
        dest="conda_env",
    ...
    if complete_h5(satg_h5f):
        ...
        if options.processes == 1:
            satg_cmd = "borzoi_satg_gene.py %s %s %s" % (
                params_file,
                model_file,
                crispr_gtf_file,
            )
...
def accuracy_stats(crispr_df, out_dir):
    ...
    for di in range(len(distance_boundaries) - 1):
        ...
        auprc_tss = average_precision_score(
            dcrispr_df.regulate, 1 / dcrispr_df.tss_distance
        )
...
def score_sites(crispr_df, gene_i, grads_ref, seq_starts, sigma=300, normalize=True):
    ...
    for ei, enh in crispr_df.iterrows():
        ...
        if eg_mid >= 0 and eg_mid < seq_len:
            ...
            escore = np.sum(weights_trunc * grads_ref_gi[eg_window]) / np.sum(
                weights_trunc
            )
            if normalize:
...
def score_sites_orig(
    crispr_df, gene_i, grads_ref, seq_starts, elen_ext=2000, normalize=True
):
    enh_ref = np.zeros(crispr_df.shape[0])
    seq_len = grads_ref.shape[1]
