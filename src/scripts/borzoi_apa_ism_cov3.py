...
'''
borzoi_apa_ism_cov3.py

Perform an ISM analysis for APA sites / UTRs specified in a csv file.
'''
...
def main():
    ...
    for gi, [_, row] in enumerate(gene_df.iterrows()):
        ...
        # get UTR start/end coordinates
        if row["strand"] == "+":
            utr_start = row["stop" + genome_suffix] - (options.pas_ext if options.pas_ext_up is None else options.pas_ext_up)
            utr_end = row["end" + genome_suffix] + 1 + options.pas_ext
        else:
            utr_start = row["end" + genome_suffix] - options.pas_ext
            utr_end = row["stop" + genome_suffix] + 1 + (options.pas_ext if options.pas_ext_up is None else options.pas_ext_up)

        gene_midpoint = (utr_start + utr_end) // 2
        ...
        genes_utr_start.append(utr_start)
        genes_utr_end.append(utr_end)
        
        # get relative coordinates
        utr_start -= gene_start
        utr_end -= gene_start
        
        # add entire UTR and all splice junctions to ISM list
        if options.full_utr and utr_start - options.window_size // 2 >= 0 and utr_end + options.window_size // 2 + 1 < seq_len :
            ism_patch_splice = []
            if options.splice_file is not None :
            ...
            ism_patch = sorted(list(set(ism_patch_splice)))
            
            # extract intervals of contiguous regions
            ism_regions = list(extract_intervals(ism_patch))
            
            genes_ism_regions.append([[utr_start, utr_end]] + ism_regions)
            ...
            ism_patch_apa = [ism_pos for ism_pos in ism_patch_apa if ism_pos >= utr_start and ism_pos <= utr_end]
        ...
        for apa_row_i, apa_row in enumerate(gene_apa_dict[row["gene"]]) :
            ...
            if apa_row["strand"] == "+" :
                bin_end = pas_bin + 1
                bin_start = bin_end - 15
            else :
                bin_start = pas_bin
                bin_end = bin_start + 15
            
            # non-distal sites
            if (row["strand"] == "+" and pas_pos < dist_pos) or (row["strand"] == "-" and pas_pos > dist_pos) :
        ...
        num_bins = np.array(sorted(list(set(num_bins))), dtype="int32")
        denom_bins = np.array(sorted(list(set(denom_bins))), dtype="int32")
    ...
    for fold_ix in options.folds :
        ...
        for cross_ix in options.crosses :
            ...
            scores_h5.create_dataset("utr_start", data=np.array(genes_utr_start))
            scores_h5.create_dataset("utr_end", data=np.array(genes_utr_end))
