...
def main():
  ...
  parser.add_option('-s', dest='splice_gff',
      default='%s/genes/gencode41/gencode41_basic_protein_splice.gff' % os.environ['HG38'],
  ...
  splice_df = pd.read_csv(options.splice_gff, sep='\t', names=['Chromosome', 'havana_str', 'feature', 'Start', 'End', 'feat1', 'Strand', 'feat2', 'id_str'], usecols=['Chromosome', 'Start', 'End', 'feature', 'feat1', 'Strand'])[['Chromosome', 'Start', 'End', 'feature', 'feat1', 'Strand']].drop_duplicates(subset=['Chromosome', 'Start', 'Strand'], keep='first').copy().reset_index(drop=True)
    ...
    for shift in options.shifts :
      ...
      for rev_comp in ([False, True] if options.rc == 1 else [False]) :
        ...
        for gi, gene_id in enumerate(gene_list):
          ...
          gene_splice_df = splice_df.query("Chromosome == '" + genes_chr[gi] + "' and ((End > " + str(genes_start[gi]) + " and End <= " + str(genes_end[gi]) + ") or (Start < " + str(genes_end[gi]) + " and Start >= " + str(genes_start[gi]) + ")) and Strand == '" + str(genes_strand[gi]) + "'").sort_values(by=['Chromosome', 'Start'], ascending=True)
          ...
          if len(gene_splice_df) > 0 :
            ...
            if donor_or_acceptor == 'donor' :
              ...
              bin_end = None
              if splice_strand == '+' :
                bin_end = int(np.round(splice_seq_start / model_stride)) + 1
                bin_start = bin_end - 3
              else :
              ...
              bin_end = None
              if splice_strand == '+' :
                bin_start = int(np.round(splice_seq_end / model_stride)) + 1
                bin_end = bin_start + 3
              else :
