...
def main():
    ...
    np.save('%s/tss_targets.npy' % options.out_dir, tss_targets)
    np.save('%s/tss_preds.npy' % options.out_dir, tss_preds)

    # save values
    tss_targets_df = pd.DataFrame(tss_targets, index=tss_ids)
    tss_targets_df.to_csv('%s/tss_targets.tsv.gz' % options.out_dir, sep='\t')
    tss_preds_df = pd.DataFrame(tss_preds, index=tss_ids)
    tss_preds_df.to_csv('%s/tss_preds.tsv.gz' % options.out_dir, sep='\t')
