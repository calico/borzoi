...
def main():
  ...
  fold_options.add_option('--name', dest='name',
      default='ipaqtl', help='SLURM name prefix [Default: %default]')
  ...
  fold_options.add_option('--vcf', dest='vcf_dir',
      default='/home/jlinder/seqnn/data/qtl_cat/ipaqtl_pip90ea')
  ...
  for ci in range(options.crosses):
    for fi in range(num_folds):
      ...
      cmd_fold = '%s time borzoi_sed_ipaqtl_cov.py %s %s' % (cmd_base, params_file, model_file)
